...
from .base import BaseAutoencoder, hidden_stack


class VAEModule(nn.Module):
    
    def __init__(self, n_inputs, n_factors, hidden_dim, depth=1, dropout_rate=0.0):
        super().__init__()
        self.trunk = nn.Sequential(*hidden_stack(n_inputs, hidden_dim, depth, dropout_rate))
        self.mu_head = nn.Linear(hidden_dim, n_factors)
        self.logvar_head = nn.Linear(hidden_dim, n_factors)
        self.decoder = nn.Sequential(
        *hidden_stack(n_factors, hidden_dim, depth, dropout_rate),
...
class VAE(BaseAutoencoder):
    ...
    def _build(self) -> nn.Module:
        return VAEModule(self.n_inputs, self.n_factors, self.hidden_dim, depth=self.depth, dropout_rate=self.dropout_rate)
